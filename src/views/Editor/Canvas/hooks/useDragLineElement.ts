...
export default (elementList: Ref<PPTElement[]>) => {
  ...
  const dragLineElement = (e: MouseEvent, element: PPTLineElement, command: OperateLineHandlers) => {
    ...
    // Get the 8 scale handles of all non-rotated elements other than lines as snapping positions
    ...
    document.onmousemove = e => {
      ...
      // Positions of the line's start and end points in the edit area
      ...
      // Position of the dragged start or end point
      // Snapping applies in both horizontal and vertical directions
      ...
      // Calculate and update the coordinates of the start and end points based on the element's own position
