...
export function HtmlThemeThumbnail({ theme, className = "" }: { theme: Pick<HtmlThemeSummary, "name" | "previewUrl" | "backgroundImageUrl" | "backgroundImageMode" | "colors" | "typography" | "effects" | "firstRecipe">; className?: string }) {
  ...
  return <div className={`relative h-full w-full overflow-hidden ${className}`} style={{ background: theme.backgroundImageUrl ? `url(${theme.backgroundImageUrl}) center / cover` : theme.backgroundImageMode === "generated" ? "linear-gradient(135deg, #062A27, #123F35 55%, #071615)" : theme.colors.background, color: theme.colors.text }}>
