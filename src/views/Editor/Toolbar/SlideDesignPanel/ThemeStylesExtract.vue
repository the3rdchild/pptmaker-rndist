  ...
  <div class="theme-styles-extract">
    ...
    <div class="content">
      ...
      <div class="config-item" v-if="themeStyles.themeColors.length">
        <div class="label">Theme color:<span class="tip">(click a color block to exclude unwanted colors)</span></div>
    ...
    <div class="btns">
      <Button class="btn" type="primary" @click="updateAllThemes()"><i-icon-park-outline:check /> Save selected as theme</Button>
    </div>
  </div>
</template>
...
const updateAllThemes = () => {
  ...
  if (themeColors.length > 6) {
    ...
    message.warning('Theme colors exceed the limit, the first 6 have been auto-selected')
  }
