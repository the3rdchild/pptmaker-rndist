    ...
    <div class="container">
      <div class="notes" ref="notesRef">
        <div class="note" :class="{ 'active': activeNoteId === note.id }" v-for="note in notes" :key="note.id" @click="handleClickNote(note)">
          ...
          <div class="note-reply" v-if="replyNoteId === note.id">
            <TextArea :padding="6" v-model:value="replyContent" placeholder="Enter a reply" :rows="1" @enter.prevent="createNoteReply()" />
      ...
      <div class="send">
        <TextArea 
          ...
          :padding="6"
          :placeholder="`Enter a note (for ${handleElementId ? 'the selected element' : 'the current slide' })`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <i-icon-park-outline:delete class="btn icon" v-tooltip="'Clear notes on this page'" style="flex: 1" @click="clear()" />
          <Button type="primary" class="btn" style="flex: 12" @click="createNote()"><i-icon-park-outline:plus /> Add Note</Button>
        </div>
      </div>
    </div>
  </MoveablePanel>
