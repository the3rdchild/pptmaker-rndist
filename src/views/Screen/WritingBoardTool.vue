    ...
    >
      <div class="tools" @mousedown.stop>
        <div class="tool-content">
          ...
          <div class="btn" v-tooltip="'Clear strokes'" @click="clearCanvas()">
        ...
        <div class="btn close" v-tooltip="'Close pen'" @click="closeWritingBoard()">
...
// Change the pen color; if currently in eraser mode, first switch to pen mode
...
// Close the writing board
const closeWritingBoard = () => {
  syncChannel.postMessage({ type: 'WRITING_BOARD_CLOSE' })
  emit('close')
}

// When opening the pen tool or switching slides, draw the strokes stored in the database onto the canvas
watch(currentSlide, () => {
...
// Sync when toggling blackboard mode
watch(blackboard, () => {
  const dataURL = writingBoardRef.value?.getImageDataURL() || ''
  broadcastWritingBoard(dataURL)
})

// After each drawing is complete, update the drawn image to the database
